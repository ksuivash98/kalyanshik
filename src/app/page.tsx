...
export default async function HomePage() {
  ...
  const [collectionCount, mixesCount, recentMixes] = await Promise.all([
    prisma.userTobacco.count({ where: { userId: user.id } }),
    prisma.mix.count({ where: { userId: user.id } }),
    prisma.mix.findMany({
  ...
  return (
    <div className="space-y-8 animate-fade-up">
      <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-stone-900 via-stone-950 to-black p-8 md:p-12">
        ...
        <div className="pointer-events-none absolute bottom-0 left-10 h-40 w-40 rounded-full bg-orange-900/30 blur-3xl" />
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle className="text-3xl">{mixesCount}</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <section className="space-y-4">
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-stone-500">Недавно сохранённые комбинации</p>
          </div>
          <Link href="/create-mix">
